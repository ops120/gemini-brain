...
agent 认出后会自己去调 `gmb`（选模型、提问、等产物、下载原图 / 源文件），你只管描述需求。

| 你想做什么 | 直接对 agent 说 |
| --- | --- |
| 高清生图（原图 2816×1536） | 「让 gemini 画一只橘猫坐在窗台上，水彩风格，我要原图」 |
| 写代码 / 页面 / 动画 | 「让 gemini 写一个纯 SVG 的鹈鹕骑自行车循环动画」「用 gemini 写个 HTML 落地页」 |
| 强推理 | 「用 gemini Pro 分析下这个报错」 |
| 分析图片 / 文件 | 「用 gemini 看看这张图里有什么」「让 gemini 读一下这份 PDF」 |
| 要可下载的源文件 | 「让 gemini 出图 / 写代码后把源文件下载到本地给我」 |

使用要点：

- **点名最稳**：话里带上「gemini」或「gmb」，agent 就会走本 skill；也支持英文触发
  （`use gemini` / `ask gemini` / `gemini image generation` / `gemini draw` / `gemini write code` /
  `gemini analyze file`）。
- **模型可选**：想用强推理就对 agent 说「用 gemini Pro」；不确定有哪些档位可以先说「列出 gemini 的可用模型」。
- **图片要下载才是原图**：页面显示的是缩略图，本 skill 会自动下载原始分辨率（`files[]` 给路径）。
- **代码类会进 Canvas 面板**：`text` 可能为空、产物在 `files[]`，属正常现象。
- **首次要先配置**：需要先安装并人工过一次 reCAPTCHA（见 [登录持久化（重要）](#登录持久化重要)），之后尽量复用；
  Google 风控要求重登时 CLI 会停下等人。
