# [GMB] 规划 / 审查协作协议（可选，实验性）

用途：让 Gemini 当「规划与审查大脑」，本地 agent 保留全部执行权。
与 deepseek-brain 的 `[DSB]`、codex-with-chatgpt 的 `[C2C]` 同构；
数据面是**推送**——Gemini 网页版不能自己拉数据。
...
checkpoint 自动写入 session（`gmb thread status --json` 可查）。
...
- **PLAN**（Gemini → 你）：应含 ACTIONS / FILES_LIKELY_INVOLVED / TESTS / SUCCESS_CRITERIA；
  只有空泛结论就要求展开一次。
- **EXECUTED**（你 → Gemini）：只报元数据（改动文件数、测试结果）+「请复核」。
  执行权永远在本地，**不要**交出去。
- **REVIEW**（Gemini）：DONE / PLAN（下一轮）/ BLOCKED。
...
- 本协议是**约定**：Gemini 不会真的执行任何操作，执行永远由本地 agent 完成。
