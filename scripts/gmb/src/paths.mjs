...
export const SERVICE = "gmb-cli";

export function stateDir() {
  const override = process.env.GMB_STATE_DIR;
  if (override && override.trim()) return path.resolve(override.trim());
  const home = os.homedir();
  if (process.platform === "win32") {
    const base = process.env.LOCALAPPDATA || path.join(home, "AppData", "Local");
    return path.join(base, "gemini-brain");
  }
  if (process.platform === "darwin") {
    return path.join(home, "Library", "Application Support", "gemini-brain");
  }
  const xdg = process.env.XDG_STATE_HOME || path.join(home, ".local", "state");
  return path.join(xdg, "gemini-brain");
}
